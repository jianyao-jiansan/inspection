...
includeMetadata=true&_limit=1&_skip=0&_sort=%5B%7B%22field%22%3A%20%22created%22%2C%20%22sortType%22%3A%20%22DESC%22%7D%5D&onlineType=ONLINE":"ONLINE", "https://rootcloud-console.rciot.bekaert.com/frame-cms/api/device/v2/thing/thing-instances?thingType=DEVICE&_\
includeMetadata=true&_limit=1&_skip=0&_sort=%5B%7B%22field%22%3A%20%22created%22%2C%20%22sortType%22%3A%20%22DESC%22%7D%5D&onlineType=OFFLINE":"OFFLINE", "https://rootcloud-console.rciot.bekaert.com/frame-cms/api/device/v2/thing/thing-instances?thingType=DEVICE&_\
includeMetadata=true&_limit=1&_skip=0&_sort=%5B%7B%22field%22%3A%20%22created%22%2C%20%22sortType%22%3A%20%22DESC%22%7D%5D&onlineType=NOT_ACTIVE":"NOT_ACTIVE", "https://rootcloud-console.rciot.bekaert.com/frame-cms/api/device/v2/thing/thing-instances?thingType=COMPOSITE_THING&_\
includeMetadata=true&_limit=1&_skip=0&_sort=%5B%7B%22field%22%3A%20%22created%22%2C%20%22sortType%22%3A%20%22DESC%22%7D%5D":"COMPOSITE_THING", "https://rootcloud-console.rciot.bekaert.com/frame-cms/api/device/v2/thing/thing-instances?thingType=GATEWAY&_includeMetadata=true&_\
limit=1&_skip=0&_sort=%5B%7B%22field%22%3A%20%22created%22%2C%20%22sortType%22%3A%20%22DESC%22%7D%5D&onlineType=ONLINE":"ONLINE_GATEWAY", "https://rootcloud-console.rciot.bekaert.com/frame-cms/api/device/v2/thing/thing-instances?thingType=GATEWAY&_includeMetadata=true&_limit=1&_\
skip=0&_sort=%5B%7B%22field%22%3A%20%22created%22%2C%20%22sortType%22%3A%20%22DESC%22%7D%5D&onlineType=OFFLINE":"OFFLINE_GATEWAY", "https://rootcloud-console.rciot.bekaert.com/frame-cms/api/device/v2/thing/thing-instances?thingType=GATEWAY&_includeMetadata=true&_limit=1&_skip=0&_\
...
url_object_dict = {"https://rootcloud-console.rciot.bekaert.com/frame-cms/api/device/devices/status/count?_groupBy=%5B%22state%22%5D":"object_instance"}
...
def url_check(url):
    try:
        ...
        if r.status_code != 200:
            # 请求响应状态异常
            msg = "%s %s" % (url_object_dict[url], r.status_code)
            ...
            #msg = "%s %s" % (url_object_dict[url], DEVICE_dict_all['payload'][0]['count'])
            #msg = "%s%s%s" % (url_object_dict[url],",", DEVICE_dict_all['payload'][0]['count'])
            msg_key = url_object_dict[url]
            msg_value = DEVICE_dict_all['payload'][0]['count']
            print("监测结果：正常")
...
#           g_two.labels('200','GET','/test/api/url/').set(10)
        ...
        # 请求响应超时
        msg = "%s请求异常: %s" % (url_object_dict[url], "请求超时", "\n\n")
        print("监测结果：超时")
