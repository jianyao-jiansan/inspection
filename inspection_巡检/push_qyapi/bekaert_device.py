...
sort=%5B%7B%22field%22%3A%20%22created%22%2C%20%22sortType%22%3A%20%22DESC%22%7D%5D&onlineType=NOT_ACTIVE":"NOT_ACTIVE_GATEWAY"}
...
def url_check(url):
    try:
        ...
        if r.status_code != 200:
            # 请求响应状态异常
            msg = "%s %s" % (url_dict[url], r.status_code)
            ...
            msg = "%s %s" % (url_dict[url], DEVICE_dict_all['metadata']['totalCount'])
            
            print("监测结果：正常")
            yuntui_push(msg)
    except requests.exceptions.ConnectTimeout:
        # 请求响应超时
        msg = "%s请求异常: %s" % (url_dict[url], "请求超时", "\n\n")
        print("监测结果：超时")
        # 通过云推推送消息
        yuntui_push(msg)
