...
ansible all -m shell -a "/bin/bash /tmp/free_info.sh" | egrep % | awk '{print $NF}'> /tmp/free_info.log
...
kubectl top no |grep -v NAME |awk '{print $5}' >>/tmp/free_info.log
free_info=$(cat /tmp/free_info.log |sort |tail -n 1)
...
kubectl top no |grep -v NAME |awk '{print $3}' >> /tmp/cpu_info.log
cpu_info=$(cat /tmp/cpu_info.log |sort |tail -n 1)
...
#K8S pod 检查开始
#kubectlexiting="/usr/local/bin/kubelet"
kubectlexiting=$(/usr/bin/which kubelet)
echo "${kubectlexiting}"
...
unready_pod=""
unnormal_pod=""
unRuninginitpodinfo="job|rook-ceph-osd-prepare-node|developer-kong|developer-internal-kong|-backup-|postgresql-stolon-create-cluster|inspectionjob-"
#K8S pod 信息写入文件
...
echo -e "Unready Pods：${unready_pod:-Ok}" >> $log_file
...
ansible all -m shell -a "df -h | egrep -v \"k8s|pods| /dev| /boot| /run| /sys| /home| /var| ^/ \"" >/tmp/disk_info.log
disk_maxrate=$(cat /tmp/disk_info.log | egrep [0-9]% | awk '{print $5}'| sort -t "%" -k1 -n | tail -n 1)
maxline=$(cat /tmp/disk_info.log |grep -n "$disk_maxrate"| awk -F":" '{print $1}')
disk_add=$(cat /tmp/disk_info.log |grep -n "$disk_maxrate" |awk '{print $NF}')
line_list=$(sed -n '/CHANGED/=' /tmp/disk_info.log)
#echo "$line_list"
j=0
for i in ${line_list}
do
  if [ "$maxline" -gt "$i" ];then
    j="$i"
    continue
  elif [ "$maxline" -lt "$i" ];then
    break
  fi
done
ip_info=$(cat /tmp/disk_info.log |head -n "$j" |tail -1 |awk '{print $1}')
...
echo -e "磁盘: 挂载点: ${disk_add} | 使用率: ${disk_maxrate} | 节点: ${ip_info} " >> $log_file
...
python /root/inspection/disk_info.py  >> $log_file

cat $log_file



cat > /tmp/system_info.json <<- EOF
{
   "cpuinfo": "${cpu_info}",
   "memoryinfo": "${free_info}",
   "diskused": "${disk_maxrate}",
   "diskonhost": "${ip_info}",
   "diskmount": "${disk_add}",
   "restartpods": "${restart_pod:-""}",
   "unreadypods":  "${unready_pod:-""}",
   "unrunningpods": "${unnormal_pod:-""}",
   "servertimestatus": ${timestatus}
}
EOF
sed -i "s@\%@@g" /tmp/system_info.json
